...
class HomeViewModelTest {
    ...
    @Before
    fun setup() {
        viewModel = HomeViewModel(getAllCafesUseCase, mockErrorEventLogger, mockBusinessEventLogger)
    }
    ...
    fun `Given success from use case but empty list returned, When #loadCafes() called, Then the viewState is Error`() {
        // Given
        val listOfCafes = emptyList<Cafe>()
    ...
    private fun givenUseCaseSuccess(cafes: List<Cafe> = listOf(feketeEntity)) = runBlocking {
        given(mockRepository.getAll()).willReturn(cafes)
    }

    private val feketeEntity = Cafe(
